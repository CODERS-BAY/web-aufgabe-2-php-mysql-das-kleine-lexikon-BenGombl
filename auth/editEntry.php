...
$statement = $connection->prepare("UPDATE `content` SET `title` = ?, `description` = ?, `teaser` = ? WHERE `content`.`id` = $lexikonId;");

$statement->bind_param("sss", $newTitle, $newDescription, $newTeaser);
$statement->execute();
$statement->close();
$connection->close();
